...
def searchForCluster (city, operadora) :
    ...
    try:
        ...
        sql = "SELECT company, cluster FROM clusters_table WHERE unaccent(city) = unaccent(%s) AND company = %s"
        cursor.execute(sql, (city, operadora))
...
def update_plan_desktop(entity_id):
    ...
    try:
        ...
        return jsonify({"error": "Erro interno no servidor", "details": str('a')}), 500
